...
from helpers import convert_seconds_to_numeric_time, build_path, add_track_to_guild_music_history
from globals_.constants import MusicVCState, MusicVCLoopMode, NEXT_MUSIC_VC_LOOP_MODE
from embed_factory import make_player_message_embed
from web_parsers.music.youtube_parser import get_youtube_track_info, get_youtube_playlist_tracks
...
class GuildMusicService:
    ...
    def __init__(self, guild_id, voice_channel_id, voice_client, text_channel):
        ...
        self.text_channel, self.music_channel, self.player_message_id =\
            get_channels_and_player_info(guild_id, text_channel)
        self.queue = list()
        self.state = MusicVCState.DISCONNECTED
        self.loop_mode = MusicVCLoopMode.NONE
        self.currently_played_track_index = int(0)
        self.current_track_progress = 0
        self.player_page = 1
        self._BASE_FFMPEG_OPTIONS = {'before_options': '-reconnect 1 ',
                                     'options': '-vn -ss 0'}
        self._force_send_notification_message = False
        self._sent_notification_initial_warning = False
        self._notifications_enabled = True
        self._adjust_indices = False
        self.reacts_last_used = 0
        self.reacts_cd_informed = False
    ...
    async def start_worker(self, pause_immediately=False):
        ...
        while self.queue and self.voice_client and self.voice_client.is_connected():
            ...
            await self._send_play_notification()
            ...
            while self.voice_client.is_playing() or self.voice_client.is_paused():
                ...
                if self.voice_client.is_playing():
                    ...
                    if self.current_track_progress % 25 == 0:
    ...
    async def add_track_to_queue(self, url, added_by, play_immediately=False, seek_to=0, refresh_player=False):
        if url not in variables.cached_youtube_info:
            variables.cached_youtube_info[url] = await get_youtube_track_info(url=url)
        if not variables.cached_youtube_info[url]:
            return None
    ...
    async def add_youtube_playlist_to_queue(self, url, added_by, channel):
        if url not in variables.cached_youtube_info:
            variables.cached_youtube_info[url] = await get_youtube_playlist_tracks(url=url)
        tracks_info = variables.cached_youtube_info[url]
        if not tracks_info:
            return None
        if len(tracks_info) > 1:
            await self.add_track_to_queue(url=tracks_info[0]['youtube_url'], added_by=added_by, refresh_player=True)
            tracks_info = tracks_info[1:]
        if tracks_info:
    ...
    async def add_tracks_to_queue(self, urls, added_by, channel):
        ...
        failed_tracks = 0
        for url in urls:
            track_info = await self.add_track_to_queue(url, added_by)
            if not track_info:
                failed_tracks += 1
    ...
    def skip_current_track(self):
        ...
        if self.loop_mode == MusicVCLoopMode.ALL and len(self.queue) == 1:
            self.queue = []
        self._force_send_notification_message = True
    ...
    def seek_to(self, seek_to_seconds):
        current_track = self.current_track
    ...
    async def initiate_dc_countdown(self):
        for _ in range(60):
            ...
            if self.voice_client.is_playing() or self.voice_client.is_paused() or not self.voice_client.is_connected():
                return
        await self.voice_client.disconnect(force=True)

    async def _send_play_notification(self):
        if self.music_channel:
            return await self.refresh_player()
        track = self.current_track
        if not self._notifications_enabled:
            return
        elif self._force_send_notification_message:
            self._force_send_notification_message = False
        elif track['seek_to']:
            return
        elif self.loop_mode == MusicVCLoopMode.ONE:
            return
        elif self.loop_mode == MusicVCLoopMode.ALL and len(self.queue) == 1:
            return
        warning_message = f"\n*This message will be sent when a new track starts playing. " \
                          f"To disable it for this session or change channel use " \
                          f"`{variables.guilds_prefs[self.guild_id].music_prefix}notify`*." \
            if not self._sent_notification_initial_warning else ""
        self._sent_notification_initial_warning = True
        fields_values = {"Duration": track['duration_text']}
        await send_embed(f"Now playing `{track.get('title')}`."
                         f"{warning_message}", self.text_channel, emoji='`▶`',
                         thumbnail_url=track['tiny_thumbnail_url'], fields_values=fields_values)

    def change_notifications_enabled_state(self, text_channel):
        different_channel = text_channel.id != self.text_channel.id
        self._notifications_enabled = True if different_channel else not self._notifications_enabled
    ...
    async def _get_current_track(self):
        ...
        if time_by_end_of_track > track_expiry_datetime:
            variables.cached_youtube_info[current_track['url']] = await get_youtube_track_info(url=current_track['url'])
            title, thumbnail_url, tiny_thumbnail_url, duration, audio_url, audio_expiry, song_details = \
                variables.cached_youtube_info[current_track['url']]
    ...
    def export_gms_to_dict(self):
        return {
            ...
            "force_send_notification_message": self._force_send_notification_message,
            "sent_notification_initial_warning": self._sent_notification_initial_warning,
    ...
    def import_gms_from_dict(self, gms_dict):
        ...
        text_channel = discord_client.get_channel(gms_dict["text_channel_id"])
        self.text_channel, self.music_channel, self.player_message_id = \
            get_channels_and_player_info(self.guild_id, text_channel)
        self.music_channel = discord_client.get_channel(gms_dict["text_channel_id"])
        self.queue = gms_dict["queue"]
        ...
        self._force_send_notification_message = gms_dict["force_send_notification_message"]
        self._sent_notification_initial_warning = gms_dict["sent_notification_initial_warning"]
        self._notifications_enabled = gms_dict["notifications_enabled"]
    ...
    async def refresh_player(self, page_adjustment=0):
        if not self.music_channel or not self.player_message_id:
            return
    ...
    async def are_reactions_on_cooldown(self):
        if datetime.utcnow().timestamp() - self.reacts_last_used < 1:
            if not self.reacts_cd_informed:
                await send_embed("Slow down!", self.music_channel, delete_after=1)
                self.reacts_cd_informed = True
            return True
        else:
            self.reacts_last_used = datetime.utcnow().timestamp()
            self.reacts_cd_informed = False


def get_channels_and_player_info(guild_id, text_channel):
    music_channel_id = variables.guilds_prefs[guild_id].music_channel
    player_message_id = variables.guilds_prefs[guild_id].music_player_message
