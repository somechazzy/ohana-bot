...
from utils.embed_factory import quick_embed, make_mal_manga_search_results_embed, make_mal_manga_info_embed
from utils.helpers import get_numbered_list_views, make_mal_info_views
from globals_.constants import Colour
from utils.helpers import get_user_stats_for_manga
...
class MangaInfoInteractionsHandler(UserInteractionsHandler):

    class SelectedView:
        INFO = "info"
        SEARCH = "search"

    def __init__(self, source_interaction, manga_info, search_result, user_stats, query):
    ...
    async def handle_selection(self, inter: discord.Interaction):
        ...
        self._manga_info = await self._mal_service.get_manga_info(
            manga_id=self._search_result['entries'][selected_index]['id']
        )
        self._user_stats = await get_user_stats_for_manga(manga_id=self._manga_info['id'],
    ...
    async def handle_cancel(self, inter: discord.Interaction):
        await inter.response.defer()
        await self.source_interaction.edit_original_response(embed=quick_embed("Manga info closed",
                                                                               color=Colour.EXT_MAL,
                                                                               bold=False),
                                                             view=None)
    ...
    def get_embed_and_views(self):
        if self._selected_view == self.SelectedView.SEARCH:
            embed = make_mal_manga_search_results_embed(query=self._query,
            ...
            embed, include_expand_button = make_mal_manga_info_embed(manga_info=self._manga_info,
