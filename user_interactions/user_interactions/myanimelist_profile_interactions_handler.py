...
import discord
from utils.embed_factory import make_mal_profile_anime_list_embed, make_mal_profile_manga_list_embed, \
    make_mal_profile_favorites_embed, make_mal_profile_embed, quick_embed
from globals_.constants import Colour
from utils.helpers import get_mal_profile_views, get_back_view, get_pagination_views
...
class MyAnimeListProfileInteractionsHandler(UserInteractionsHandler):
    ...
    def __init__(self, source_interaction, profile_info, anime_list, manga_list, username, unlocked):
    ...
    async def handle_cancel(self, inter: discord.Interaction):
        if self.source_interaction.user.id != inter.user.id:
            if not (self.guild and self.guild.get_member(inter.user.id).guild_permissions.manage_messages):
                return await inter.response.defer()
        await inter.response.defer()
        await self.source_interaction.edit_original_response(embed=quick_embed("MAL Profile closed",
    ...
    def get_embed_and_views(self):
        if self._selected_view == self.SelectedView.ANIME_LIST:
            embed = make_mal_profile_anime_list_embed(username=self._username,
                                                      profile_avatar=self._profile_info["profile_avatar"],
            ...
            embed = make_mal_profile_manga_list_embed(username=self._username,
                                                      profile_avatar=self._profile_info["profile_avatar"],
            ...
            embed = make_mal_profile_favorites_embed(profile_info=self._profile_info)
            views = get_back_view(interactions_handler=self)
        else:
            embed = make_mal_profile_embed(profile_info=self._profile_info,
                                           reacting_unlocked=self._unlocked)
            views = get_mal_profile_views(add_unlock=not self._unlocked,
                                          interactions_handler=self)
