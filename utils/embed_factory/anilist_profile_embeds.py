...
from globals_.constants import AnilistStatus, Colour
...
def make_anilist_profile_embed(profile_info: AnilistProfile, reacting_unlocked=False):
    if profile_info is None:
        embed = discord.Embed(title=f"Error getting profile", colour=Colour.EXT_ANILIST,
    ...
    embed = discord.Embed(title=f"{username}", colour=Colour.EXT_ANILIST, url=f"{profile_url}")
    ...
    embed.add_field(name="User Info",
                    value=
                    (f"Joined Anilist: <t:{int(anilist_user.created_at)}:D>\n" if anilist_user.created_at else "")
...
def make_anilist_profile_anime_list_embed(username, profile_avatar, anime_list, page, total_pages, scoring_system):
    list_url = f"https://anilist.co/user/{username}/animelist"
    profile_url = f"https://anilist.co/user/{username}"
    
    embed = discord.Embed(title=f"{username}'s anime list", colour=Colour.EXT_ANILIST, url=f"{list_url}")
    embed.set_thumbnail(url=f"{profile_avatar}")
    embed.set_author(name="Anilist profile", url=f"{profile_url}",
                     icon_url="https://anilist.co/img/icons/android-chrome-512x512.png")
    ...
    if not total_pages:
        ...
        embed.set_footer(text=f"Page {page}/{total_pages} | Total of {len(anime_list)} anime entries| Scoring system:"
                              f" {scoring_system}")
    ...
    for i, entry in enumerate(anime_list[starting_index:final_index], starting_index):
        anime_name = entry.media.title.romaji
        anime_url = entry.media.site_url
        anime_rating = f" - {entry.score}" if entry.score != 0 else ''
        anime_status = str(entry.status.name).lower().capitalize()
...
def make_anilist_profile_manga_list_embed(username, profile_avatar, manga_list, page, total_pages, scoring_system):
    list_url = f"https://anilist.co/user/{username}/mangalist"
    profile_url = f"https://anilist.co/user/{username}"
    if total_pages == 0:
        embed = discord.Embed(title=f"{username}'s manga list", colour=Colour.EXT_ANILIST, url=f"{list_url}",
                              description="‎‎‎\nThis list is empty!\n‎‎‎")
    ...
    embed.set_author(name="Anilist profile", url=f"{profile_url}",
                     icon_url="https://anilist.co/img/icons/android-chrome-512x512.png")
    embed.set_footer(text=f"Page {page}/{total_pages} | Total of {len(manga_list)} manga entries | Scoring system:"
                          f" {scoring_system}")
    starting_index = 0 + (page - 1) * 10
...
def make_anilist_profile_favorites_embed(profile_info):
    ...
    embed = discord.Embed(title=f"{username}'s favorites", colour=Colour.EXT_ANILIST, url=f"{profile_url}")
    ...
    if exceeded_max_length:
        embed.add_field(name="Too many favorites!!", value=f"Not all of your favorites are shown because they are too"
                                                           f" many. Not showing them is a temporary fix and in the"
                                                           f" near future we'll figure out a way to display them all."
                                                           f" Sorry for now :(")
    return embed
